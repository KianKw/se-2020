...
public class Solution extends Jigsaw {
    ...
    public boolean BFSearch(JigsawNode bNode, JigsawNode eNode) {
        Set<JigsawNode> hashJigsaw = new HashSet<>(1000);    // 用以保存已发现的节点
        Queue<JigsawNode> que = new LinkedList<JigsawNode>();

        que.offer(bNode);
        while (!que.isEmpty()) {
            // 取出exploreList的第一个节点N，置为当前节点currentJNode
            // 若currentJNode为目标节点，则搜索成功，计算解路径，退出
            this.currentJNode = que.poll();
            if (this.currentJNode.equals(eNode)) {
                ...
                for (int i = 0; i < 4; i++) {
                    if (nextNodes[i].move(i) && !hashJigsaw.contains(nextNodes[i])) {
                        hashJigsaw.add(nextNodes[i]);
                        que.offer(nextNodes[i]);
                    }
                }
            }
        }  
    ...
    public void estimateValue(JigsawNode jNode) {
        ...
        int ans = (int)(53 * s + 71 * manHatten + 117 * geometric + 315 * manHatten % s + 911 * geometric % s);
		long tt = (long)ans;
		if (tt > Integer.MAX_VALUE) {
			System.out.println("out of int error");
		}
        jNode.setEstimatedValue(ans);
    }
}